...
// edge cases or not) on Win7 with VS2015 these extern declaration cause
// "second C linkage of overloaded function not allowed" error
typedef union
{
  struct
  {
  ...
  {                                                                           \
    ...
    try                                                                       \
      ...
      {                                                                       \
        ...
        exception_caught = 1;                                                 \
        if (0                                                                 \
            != UTEST_STRNCMP (what, exception_message,                        \
      ...
      if (0 == UTEST_STRNCMP (argv[index], help_str, strlen (help_str)))
        {
          ...
          printf (
              ...
              "  --random-order[=<seed>] Randomize the order that the tests "
              "are "
              "ran in. If the optional <seed> argument is not provided, then "
              "a "
      ...
      else if (0 == UTEST_STRNCMP (argv[index], list_str, strlen (list_str)))
        {
          ...
          return 0;
               ...
               == UTEST_STRNCMP (argv[index], enable_mixed_units_str,
                                 strlen (enable_mixed_units_str)))
        {
          enable_mixed_units = 1;
        }
      else if (0
               == UTEST_STRNCMP (argv[index], random_order_with_seed_str,
                                 strlen (random_order_with_seed_str)))
        {
          ...
          random_order = 1;
               ...
               == UTEST_STRNCMP (argv[index], random_order_str,
                                 strlen (random_order_str)))
        {
          ...
          // mean our random would be affected by any srand() usage by the user
          // (which I don't want).
